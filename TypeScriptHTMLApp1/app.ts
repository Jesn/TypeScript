...
enum Color2 { Red = 1, Green, Blue, Yellow };

class Human {

    //对于基本类型的批注是number, bool 和string。
    //而弱或动态类型的结构则是any 类型。
    //当类型没有给出时，TypeScript 编译器利用类型推断以推断类型。
    //如果由于缺乏声明，没有类型可以被推断出，那么它就会默认为是动态的any 类型。
    UserName: string;
    Age: number;
    Sex: number;
    Nice: any;
    ...
    Behavior(username: string, age: number, sex: number,nice:any) {
        ...
        var c: Color = Color.Blue;//从枚举里面拿出Blue出来赋给一个叫c 的变量
        ...
        //如一个混合了多种类型的集合数组
        var notSure: any = 4;//notsure是一个不确定的值，默认先给一个数字类型4
...
/**获取数组类型为any的值*/
function getColorList(): void {
    var colorList: Array<any> = ["Green", "Yellow", "Blue"];//any 不知道类型
